...
class PaperTraderV5 {
      ...
      if (result.total >= 3) { // Minimum 3 trades for validation
        ...
        if (wr >= 65) {
          ...
          negativeStrategies.push({
            ...
            name: result.name,
            winRate: wr.toFixed(2),
            reason: wasNegative ? 'Still below 65% after re-test' : 'WR below 65%',
            cycle: currentCycle,
            trades: result.total,
            canReTest: true
          });
  ...
  writePositiveStrategies(strategies, currentCycle) {
    let content = `# 16 - Positive Strategies (WR >=65%)\n\n`;
    ...
    if (strategies.length === 0) {
      content += `*No strategies currently meet the 65% WR threshold*\n\n`;
    }
    ...
    content += `---\n\n`;
    content += `**Note:** Strategies are valid for 1 cycle only. After each 50-simulation cycle, all strategies are re-evaluated. Only strategies maintaining ≥65% WR remain in Positive.\n`;
    
    fs.writeFileSync(CONFIG.POSITIVE_STRATEGIES_FILE, content);
  }

  writeNegativeStrategies(strategies, currentCycle) {
    let content = `# 17 - Negative Strategies (WR <65%)\n\n`;
    content += `**Auto-generated by Paper Trader v5**\n`;
    content += `**Updated:** ${new Date().toISOString()}\n`;
    content += `**Current Cycle:** ${currentCycle}\n\n`;
  ...
  calculatePositionSizingRules() {
    ...
    for (const [strategyId, result] of Object.entries(this.results)) {
      if (result.total >= 5) {
        ...
        else if (wr >= 65) rules[strategyId] = '0.04';
  ...
  async notifyTelegram() {
    try {
      ...
      msg += `\n📚 BOK: ${positiveCount} strategies ≥65% WR\n`;
